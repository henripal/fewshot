...
### adapted from @oscarknagg's repo https://github.com/oscarknagg/few-shot/blob/master/few_shot/maml.py
...
def meta_gradient_step(model: Module,
    ...
    """
    x = x.reshape(-1, n_shot * k_way + q_queries * k_way, *x.shape[-3:])
    ...
    for meta_batch in x:
        ...
        for inner_batch in range(inner_train_steps):
            ...
            y = torch.arange(0, k_way, 1/n_shot).long().to(device)
        ...
        y = torch.arange(0, k_way, 1/q_queries).long().to(device)
    ...
    if order == 1:
        if train:
            ...
            loss = loss_fn(logits, torch.arange(0, k_way, 1).long().to(device))
            loss.backward()
            optimiser.step()
